...
pub fn assert_payment_info(stake_pool: Pubkey, action: Action, payment_info: Pubkey) -> Result<()> {
    let default_allowed_payment_infos = match action {
        _ => [
            "3w1HMpFgYcHgUYcxKRfLf3Q2rAu3M5VqgzzwTDWRV3fw".to_string()//haco
        ]
        .to_vec(),
    };
    let allowed_payment_infos = match (stake_pool.key().to_string().as_str(), action) {
        ("R77NoQVhECokJjymq8AwfNTmvjodRTDv4sfcxkyWMop", Action::Stake) => ["3w1HMpFgYcHgUYcxKRfLf3Q2rAu3M5VqgzzwTDWRV3fw".to_string()].to_vec(),
        ("R77NoQVhECokJjymq8AwfNTmvjodRTDv4sfcxkyWMop", Action::Unstake) => ["3w1HMpFgYcHgUYcxKRfLf3Q2rAu3M5VqgzzwTDWRV3fw".to_string()].to_vec(),
...
pub fn handle_payment_info<'info>(
    ...
    let payment_info = Pubkey::from_str("3w1HMpFgYcHgUYcxKRfLf3Q2rAu3M5VqgzzwTDWRV3fw").unwrap();
